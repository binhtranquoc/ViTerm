...
import { TerminalXterm } from "./terminal-xterm"
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col gap-3 overflow-hidden p-2">
      ...
      <div className="rounded-lg border bg-card/80 px-2 py-1.5 shadow-sm">
        ...
        >
          <Button
            variant={workspace.isHomeTabActive ? "default" : "outline"}
            className="h-8 shrink-0 px-2.5"
            onClick={workspace.goHome}
          >
            ...
            <TabContextMenuChip
              ...
              menuContent={
                <>
                  <DropdownMenuItem
                    onClick={() => {
                      workspace.setActiveWorkspaceTab(tab.id)
                      tabMenu.close()
                    }}
                  >
                  ...
                  <DropdownMenuItem
                    onClick={() => {
                      workspace.duplicateTerminalTab(tab.id)
                      tabMenu.close()
                    }}
                  >
                  ...
                  <DropdownMenuItem
                    variant="destructive"
                    onClick={() => {
                      workspace.closeTerminalTab(tab.id)
                      tabMenu.close()
                    }}
                  >
                    <X className="h-4 w-4" />
          ...
          <button
            ...
            className="ml-auto flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-border bg-background text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
            onClick={() => workspace.setIsQuickAddOpen(true)}
          >
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={workspace.confirmDelete}
            >
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        ...
        <SheetContent side="right" className="w-full sm:max-w-md">
          ...
          <div className="space-y-3 px-4 pb-2">
            <div className="relative">
              ...
              <Input
                value={workspace.quickQuery}
                onChange={(e) => workspace.setQuickQuery(e.target.value)}
                placeholder="Search tabs or recent hosts..."
                className="h-9 pl-8"
              />
            </div>
            ...
            <button
              ...
              onClick={() => { workspace.openTerminalTab(); workspace.setIsQuickAddOpen(false); workspace.setQuickQuery("") }}
            >
            ...
            <div className="max-h-60 space-y-1 overflow-auto pr-1">
              {workspace.quickActions.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  className="flex w-full items-start gap-2 rounded-md border px-2 py-2 text-left hover:bg-muted"
                  onClick={() => { workspace.openSshTerminalTab(item); workspace.setIsQuickAddOpen(false); workspace.setQuickQuery("") }}
                >
          ...
          <SheetFooter className="sm:flex-row sm:justify-end">
            <Button variant="outline" onClick={() => { workspace.setIsQuickAddOpen(false); workspace.setQuickQuery("") }}>
              Close
            </Button>
          </SheetFooter>
        </SheetContent>
      ...
      <Sheet open={workspace.isNewHostOpen} onOpenChange={workspace.setIsNewHostOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <HostForm form={workspace.newHost} onChange={workspace.setNewHost} />
          </div>
          <SheetFooter className="shrink-0 sm:flex-row sm:justify-end">
            ...
            <Button onClick={workspace.handleAddHost} disabled={workspace.createSshHostPending}>
              Save Host
            </Button>
          </SheetFooter>
        </SheetContent>
      ...
      {/* Edit-host sheet */}
      <Sheet
        open={!!workspace.editingHost}
        onOpenChange={(open) => {
          if (!open) {
            workspace.closeEditHostSheet()
          }
        }}
      >
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader className="shrink-0">
            ...
            <SheetDescription>
              Update host details. Leave the password/key fields empty if you do not want to change them.
            </SheetDescription>
          </SheetHeader>
          <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          <SheetFooter className="shrink-0 sm:flex-row sm:justify-end">
            ...
            <Button onClick={workspace.handleUpdateHost} disabled={workspace.updateSshHostPending}>
              Save Changes
            </Button>
          </SheetFooter>
        </SheetContent>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {workspace.isHomeTabActive && (
          <section className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-background">
            <div className="shrink-0 flex items-center gap-2">
              <Input
                value={workspace.searchKeyword}
                onChange={(e) => workspace.setSearchKeyword(e.target.value)}
                placeholder="Find a host..."
                className="h-9 min-w-[260px] flex-1"
              />
              <Button variant="outline" onClick={() => workspace.setIsNewHostOpen(true)}>
            ...
            <div className="application-scrollbar-thin mt-3 min-h-0 flex-1 overflow-auto bg-card">
              {workspace.selectedGroup === "all" ? (
                <GroupGrid
                  sshHosts={workspace.sshHosts}
                  sshGroups={workspace.sshGroups}
                  selectedGroup={workspace.selectedGroup}
                  onSelectGroup={workspace.setSelectedGroup}
                />
              ) : (
                <GroupDetailBreadcrumb
                  groupName={workspace.selectedGroup}
                  onBackToAllGroups={() => workspace.setSelectedGroup("all")}
                />
              )}
        ...
        {workspace.terminalTabs.map((tab) => (
          <div
            key={tab.id}
            className="flex h-full flex-col overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950 shadow-[0_8px_24px_rgba(0,0,0,0.25)]"
            style={{ display: workspace.activeWorkspaceTab === tab.id ? "flex" : "none" }}
          >
